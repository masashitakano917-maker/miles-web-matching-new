...
type ConfirmState = {
  service: string;        // 'photo' | 'clean' | 'staff'
  plan_key: string;       // '20' | '30' | '1ldk' など
  plan_title: string;     // 表示用
  price: number;

  prefer_datetime_1?: string; // ISO
    ...
    try {
      ...
      const res = await fetch('/api/matching?action=create_request', {
        ...
        body: JSON.stringify({
          ...
          // 希望日時
          prefer_datetime_1: payload.prefer_datetime_1,
          prefer_datetime_2: payload.prefer_datetime_2,
          prefer_datetime_3: payload.prefer_datetime_3,
        }),
      });
      ...
      <main className="max-w-4xl mx-auto px-4 py-10">
        ...
        <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm mt-6">
          ...
          <div className="mt-2 text-gray-800">
            {payload.service} / {payload.plan_title}（{payload.price.toLocaleString()}円）
          </div>
        </section>
