      ...
      <main className="container mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 pt-10 pb-20">
        ...
        {isPro ? (
          ...
          <div className="rounded-2xl border border-gray-100 bg-white shadow-sm p-6">
            ...
            {/* 新規発注はまずサービス一覧（/services）へ */}
            <div className="mt-6 flex flex-wrap gap-3">
              <Link to="/services?tab=photo" className="btn-primary">新規発注</Link>
              <Link to="/services?tab=clean" className="btn-ghost">清掃サービスを見る</Link>
              <Link to="/services?tab=staff" className="btn-ghost">人材派遣を見る</Link>
            </div>

            <p className="text-sm text-gray-500 mt-3">
              各サービスの「オーダーへ」ボタンから、発注フォーム（/order）に進みます。
            </p>
          </div>
        )}
      </main>
